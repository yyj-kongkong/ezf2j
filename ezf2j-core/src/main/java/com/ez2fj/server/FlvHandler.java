...
public class FlvHandler extends SimpleChannelInboundHandler<Object> {
	 ...
	 * 网络超时
	 */
	private String netTimeout = EZF2JConfig.NET_TIMEOUT;;
	/**
	 * 读写超时
	 */
	private String readOrWriteTimeout = EZF2JConfig.READ_OR_WRITE_TIMEOUT;

	/**
	 * 无人拉流观看是否自动关闭流
	 */
	private boolean autoClose = EZF2JConfig.AUTO_CLOSE;

	/**
	 * 无人拉流观看持续多久自动关闭，1分钟
	 */
	private long noClientsDuration= EZF2JConfig.NO_CLIENTS_DURATION;;
...
//			Map<String, String> parmMap = new RequestParser(msg).parse();
			...
			// 判断请求uri
			if (!EZF2JConfig.FLV_PATH.equals(decoder.path())) {
				log.info("uri有误");
			...
			if (!req.decoderResult().isSuccess() || (!"websocket".equals(req.headers().get("Upgrade")))) {
				...
				if (handshaker == null) {
					...
					rsp.headers().set(HttpHeaderNames.SERVER, EZF2JConfig.serverName);
	...
	private void sendFlvReqHeader(ChannelHandlerContext ctx) {
		...
		rsp.headers().set(HttpHeaderNames.CONNECTION, HttpHeaderValues.CLOSE)
				...
				.set(HttpHeaderNames.TRANSFER_ENCODING, HttpHeaderValues.CHUNKED).set(HttpHeaderNames.SERVER, EZF2JConfig.serverName);
